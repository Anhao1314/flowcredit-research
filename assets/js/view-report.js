...
(function () {
  ...
  function customReport(host, draft) {
    ...
    host.innerHTML = '<div class="v-page custom-report">' + ui.pageHead('03 / REPORT', draft.input.label || 'Custom operator', 'A Token-adjusted risk screen based only on the facts supplied in this session.') +
      '<div class="v-action-row report-toolbar"><button class="btn btn-primary" id="report-print" type="button">Print / Save PDF</button><button class="btn" id="report-export" type="button">Download JSON</button><a class="btn btn-ghost" href="#/ingest">Edit input</a></div>' +
      ...
      '<section class="v-panel" id="report-section-1" tabindex="-1">' + App.resultSummaryHtml(run,draft) + '<div class="v-metrics">' + ui.metric('TAI', shown(run.tai), run.tokenActivityBand || 'Not computable') + ui.metric('CCI', shown(run.cci), 'Experimental risk rules') + ui.metric('Rule risk grade',shown(run.grade),'Manual review only') + ui.metric('Evidence readiness',ui.esc(run.evidenceStrength || 'not rated'),run.tokenMeteringStatus || 'not computable') + '</div><p>' + ui.esc(run.trace || '') + '</p></section>' +
      ...
      '<section class="v-panel" id="report-section-6" tabindex="-1"><div class="v-section-head"><div><p class="v-eyebrow">6 · METHODOLOGY AND LIMITATIONS</p><h2>What this result does not claim</h2></div></div><div class="fc-report-actions">' + App.actionsHtml(run.requiredActions) + '</div><ul class="intake-issues">' + (run.limitations || []).map(function (item) { return '<li>' + ui.esc(item) + '</li>'; }).join('') + '</ul><h3>D. Limitations and disclaimer</h3><p class="v-caption">This is a conservative risk screen, not a statutory audit, lending decision or financial advice. TAI measures activity coherence, not revenue or creditworthiness.</p></section>' +
