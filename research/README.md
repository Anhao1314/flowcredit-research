...
Local Hybrid Retrieval v0.11 adds the [span-level retrieval layer](local-retrieval/README.md) that narrows the SourceSupport candidate set with local BM25 + local embeddings + RRF before the unchanged selection call, the [pre-code audit](local-retrieval/REPOSITORY_AUDIT.md) that measured the v0.10 context cost, a pre-registered retrieval gate and the A–Y decision recorded in the [v0.11 delivery](../docs/local-hybrid-retrieval-v0.11.md).

Local LLM Validation v0.10 adds the loopback-only [LocalProvider and benchmark pipeline](local-model/README.md): an Ollama-backed open-weight model carrying the same restricted Evidence Analyst role with no paid inference API, the frozen DeepSeek v0.9 baseline as the comparison point and a pre-registered local viability gate. The [hardware audit](local-model/HARDWARE_AUDIT.md), sanitized artifacts and the A–T decision are recorded in the [v0.10 delivery](../docs/local-llm-validation-v0.10.md).
Model-facing Selection Handles v0.11.1 replaces the canonical span id in the [selection prompt](selection-handles/README.md) with an invocation-local handle resolved by exact deterministic code, freezing the v0.11 retrieval, K=8, model and prompts' semantic task. The [pre-code audit](selection-handles/REPOSITORY_AUDIT.md) and the A–W delivery with the `STAY ON HANDLE HARDENING` decision are recorded in [the v0.11.1 delivery](../docs/model-facing-selection-handles-v0.11.1.md).
